...
class RegistroDuenioPage extends StatefulWidget {
  final String token;
  const RegistroDuenioPage({super.key, required this.token});

  @override
  State<RegistroDuenioPage> createState() => _RegistroDuenioPageState();
}

class _RegistroDuenioPageState extends State<RegistroDuenioPage> {
  ...
  Future<void> _verificarToken() async {
    try {
      final res = await http.get(
        Uri.parse('$_base/api/auth/verificar-invitacion-duenio?token=${widget.token}'),
      );
      if (!mounted) return;
      if (res.statusCode == 200) {
        ...
        setState(() {
          _tokenValido = true;
          _tipoTienda  = data['tipo_tienda'] as String?;
          _verificando = false;
        });
      } else {
        final data = jsonDecode(res.body);
  ...
  Future<void> _registrar() async {
    ...
    try {
      final res = await http.post(
        Uri.parse('$_base/api/auth/registro-duenio'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          'token':        widget.token,
          'nombre':       _nombreCtrl.text.trim(),
          'nombre_tienda': _nombreTiendaCtrl.text.trim(),
      ...
      if (res.statusCode == 201) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('¡Cuenta y tienda creadas! Ya puedes iniciar sesión.'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(data['error'] ?? 'Error al crear cuenta'),
      ...
      children: [
        ...
        if (_tipoTienda != null) ...[
          Container(
            ...
            child: Text(
              'Registrando tienda de tipo: $_tipoTienda',
          ...
          child: Column(
            children: [
              ...
              TextFormField(
                ...
                decoration: _inputDec('Tu nombre completo', Icons.person_outline),
              ...
              TextFormField(
                ...
                style: const TextStyle(color: Colors.white),
                decoration: _inputDec('Nombre de tu tienda', Icons.store_outlined),
                validator: (v) => (v == null || v.trim().isEmpty) ? 'Campo requerido' : null,
              ),
              const SizedBox(height: 12),
              ...
              TextFormField(
                ...
                style: const TextStyle(color: Colors.white),
                decoration: _inputDec('Descripción (opcional)', Icons.notes_outlined),
                maxLines: 2,
              ),
              const SizedBox(height: 12),

...
class RegistroTrabajadorPage extends StatefulWidget {
  final String token;
  const RegistroTrabajadorPage({super.key, required this.token});

  @override
  State<RegistroTrabajadorPage> createState() => _RegistroTrabajadorPageState();
}

class _RegistroTrabajadorPageState extends State<RegistroTrabajadorPage> {
  ...
  Future<void> _verificarToken() async {
    try {
      final res = await http.get(
        Uri.parse('$_base/api/verificar-invitacion?token=${widget.token}'),
      );
      if (!mounted) return;
      if (res.statusCode == 200) {
        ...
        setState(() {
          _tokenValido  = true;
          _nombreTienda = data['tienda'] as String?;
          _verificando  = false;
        });
      } else {
        final data = jsonDecode(res.body);
  ...
  Future<void> _registrar() async {
    ...
    try {
      final res = await http.post(
        Uri.parse('$_base/api/registro-trabajador'),
      ...
      children: [
        ...
        if (_nombreTienda != null) ...[
          Container(
            ...
            child: Text(
              'Invitado a unirte a $_nombreTienda',
